...
Силовые (упражнение → вес × повторы), можно списком:
```
жим 80x2
жим 80x2, присед 100x5, тяга 120x3
```
Напиши только имя (`жим`) — бот предложит повторить прошлый подход кнопками
[✅ Повторить] / [✏️ Изменить].

Команды: `/week`, `/month`, `/today`, `/setgoal`, `/goal`, `/strength <упр>`,
`/trening` (топ упражнений кнопками), `/exercises`.
